...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-u", "--user", dest="user", default=None, help="username for webservice"
    )
    parser.add_argument(
        "-p", "--password", dest="pw", default=None, help="password for webservice"
    )
    ...
    if args.user:
        ...
        if not args.pw:
            raise argparse.ArgumentError(f"Missing password with provided user?")
    if args.pw:
        Config.STORE_PASS = args.pw
        if not args.pw:
            raise argparse.ArgumentError(f"Missing user with provided password?")
